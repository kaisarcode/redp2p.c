...
> **Real state as of 2026-08-07 (refactor session resumed):** Phases 1, 2, 3, and 5 are
...
> all work over HTTP. Remaining: Phase 4 (`REDP2P_INDEX_URL` + default URL resolution),
...
- [ ] `REDP2P_INDEX_URL` environment variable honored
- [ ] Default URL derived from `id@host[:port]`
...
- [ ] `README.md` documents `REDP2P_INDEX_URL` and the HTTP endpoint model
- [ ] `DESIGN.md` documents the stateless HTTP contract, TTL/prune, direct punch, key stability, NAT limitation
- [ ] `AGENTS.md` core invariant updated from TCP control channel to HTTP control API
